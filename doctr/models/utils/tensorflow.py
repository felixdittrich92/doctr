...
def load_pretrained_params(
    ...
    **kwargs: Any,
    ...
    if url is None:
        ...
        # skip mismatch to allow fine-tuning on differing vocabularies (same for KIE models)
        model.load_weights(archive_path, skip_mismatch=True)
...
class IntermediateLayerGetter(Model):
    ...
    def __init__(self, model: Model, layer_names: List[str]) -> None:
        intermediate_fmaps = [model.get_layer(layer_name).output[0] for layer_name in layer_names]
