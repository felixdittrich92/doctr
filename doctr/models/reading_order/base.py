...
def _geometry_to_xyxy(geometry: Any) -> tuple[float, float, float, float]:
    """Convert any supported docTR geometry (2-point box or 4-point polygon) to (xmin, ymin, xmax, ymax)"""
    pts = np.asarray(geometry, dtype=np.float64).reshape(-1, 2)
    return float(pts[:, 0].min()), float(pts[:, 1].min()), float(pts[:, 0].max()), float(pts[:, 1].max())


def _to_boxes(geoms: Sequence[Any] | np.ndarray) -> np.ndarray:
    """Convert a sequence of geometries (or a (N, 4) / (N, 4, 2) array) to a (N, 4) xyxy float array"""
...
def sort_reading_order(
    ...
    >>> from doctr.utils.reading_order import sort_reading_order
...
class ReadingOrderPredictor(NestedObject):
    ...
    >>> import numpy as np
    >>> from doctr.utils.reading_order import ReadingOrderPredictor
