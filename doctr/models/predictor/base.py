...
class _OCRPredictor:
    ...
    def _mask_regions(
        ...
        """Blank out layout regions that must be skipped by detection/recognition.

        Regions whose class name is in `self.ignore_regions` (e.g. `Picture` or `Formula`) are filled
        with the specified `fill_value` so the text detection model finds nothing inside them.

        Args:
            pages: the page images
            regions: per-page layout predictions `{"class_names", "boxes", "scores"}` (or `None`)
            fill_value: pixel value used to fill the masked regions

        Returns:
            the pages with the selected regions masked out (inputs are left untouched)
        """
