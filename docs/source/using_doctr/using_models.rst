...
:meth:`Page.items_in_reading_order` returns the blocks & tables of a page in reading order. Rotated pages
(4-point geometries) are de-skewed internally before ordering, so the reading order is robust to skew out of
the box; the exporters themselves live in :mod:`doctr.io.exporters` (:class:`~doctr.io.MarkdownExporter`,
:class:`~doctr.io.AsciiDocExporter`, :class:`~doctr.io.HTMLExporter`) and can be used directly.
