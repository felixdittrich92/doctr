...
def test_sort_reading_order_fragmented_columns():
    # A two-column layout whose left column contains a line split into a narrow part and a stray fragment
    # (as real OCR frequently produces). The whole left column must still be read before the right one,
    # rather than switching columns where the top-to-bottom chain is momentarily broken.
    left = [
